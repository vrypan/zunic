"""Reachable semantic histories for the Unicode 17 line-break compiler.
...
def resolve(state, category):
    ...
    if raw == "cj":
        return "ns"
    if raw == "sa":
        raw = "cm" if category.sa_mark else "al"
    if raw in MARK:
...
def consume(state, category, *, first=False):
    ...
    # LB1 resolves SA marks (General_Category Mn/Mc) to CM. From this point
    # they participate in the same LB9 inheritance and history lifetime as an
    # explicit CM; retaining the raw SA value is only for diagnostics.
    mark = raw in MARK or raw == "sa" and category.sa_mark
    ...
    return History(
        ...
        hl_hy=p == "hl" and c in {"hy", "hh"},
        initial_hy=(c in {"hy", "hh"} or category.hyphen) if first else (
            state.initial_hy if mark else (c in {"hy", "hh"} or category.hyphen) and p in WORD_START),
...
def decision(state, category, following=None, second_nu=False):
    """Ordered Unicode 17 decisions, evaluated only by the offline compiler.
    ...
    if state.raw == "zwj":
        return 0
    mark = raw in MARK or raw == "sa" and category.sa_mark
    if mark and p != "sp" and state.raw != "zw":
        return 0
    if p == "wj" or c == "wj" or p == "gl":
        return 0
    if c == "gl" and p not in {"sp", "ba", "hy", "hh"}:
        return 0
    ...
    if p == "cb" or c == "cb":
        return 1
    if c in {"ba", "hh", "hy", "ns"} or p == "bb":
        return 0
    if state.hl_hy and c != "hl" or state.initial_hy and c in {"al", "hl"} or p == "sy" and c == "hl":
        return 0
...
def render():
    ...
    return ("//! Generated by src/tools/generate-line-break-machine.py --write; do not edit.\n"
            f"// Unicode 17: {len(states)} reachable histories, {len(rows)} minimized states.\n"
