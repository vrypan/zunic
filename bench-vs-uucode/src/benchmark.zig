...
inline fn run(comptime operation: Operation, case: Case) peer.Stats {
    ...
    return switch (operation) {
        ...
        .bidi_properties => peer.bidiProperties(codepoints),
        .bidi_mappings => peer.bidiMappings(codepoints),
...
fn dumpOne(out: *std.Io.Writer, case: Case, comptime operation: Operation) !void {
    ...
    switch (operation) {
        ...
        .bidi_properties => try peer.dumpBidiProperties(out, case.codepoints),
        .bidi_mappings => try peer.dumpBidiMappings(out, case.codepoints),
